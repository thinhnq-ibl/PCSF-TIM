...
| **Supporting Claim** | **Purpose** | **Representative Evidence (SOTA)** | **Expected Conclusion** |
| --- | --- | --- | --- |
| **Claim B1. Human mobility exhibits a general distance-decay regularity: aggregate spatial interaction tends to decrease with increasing spatial distance.** | Giới thiệu khái niệm trung tâm "distance-decay regularity" làm chủ đề xuyên suốt Module B, thiết lập tiến trình từ nền tảng địa lý/vật lý đến thực nghiệm Big Data. | • Tobler (1970): First Law of Geography.<br>• Haynes & Fotheringham (1984): Spatial Interaction Theory.<br>• Wilson (1971): Entropy-maximizing derivation.<br>• González et al. (2008) & Song et al. (2010): Individual trajectories & predictability.<br>• Barbosa et al. (2018) & Gallotti et al. (2024): Modern mobility reviews. | Distance-decay là quy luật thực nghiệm phổ quát, hình thành không ngẫu nhiên và được duy trì bền vững trên mọi nguồn dữ liệu di chuyển. |
| **Claim B2. Gravity models explicitly represent distance-decay through a parameterized deterrence function.** | Dẫn dắt từ hiện tượng thực nghiệm (B1) sang biểu diễn toán học tường minh thông qua hàm tham số $f(d_{ij}; \theta)$. | • Zipf (1946).<br>• Wilson (1971).<br>• Erlander & Stewart (1990). | Hàm distance-decay là công cụ toán học để thể hiện quy luật suy giảm khoảng cách thực nghiệm. |
| **Claim B3. The distance-decay function is the mathematical representation of collective distance sensitivity.** | Giải thích ý nghĩa hành vi của hàm decay và các tham số $\theta$ trong bối cảnh cấu trúc đô thị. | • Tanner (1961).<br>• Hansen (1959).<br>• Lenormand et al. (2016). | Tham số $\theta$ chính là đối tượng hành vi trung tâm cần nhận diện. |
| **Claim B4. Different functional forms represent different hypotheses about collective distance sensitivity, with Tanner providing a flexible dual-parameter representation.** | Giải thích sự khác biệt giữa các hàm deterrence và lý do lựa chọn hàm Tanner ($d^{-\alpha}e^{-\beta d}$). | • Exponential & Power-law models.<br>• Tanner (1961).<br>• Liang et al. (2013).<br>• Lenormand et al. (2016). | Hàm Tanner có khả năng mô tả đồng thời cả near-distance preference ($\alpha$) và long-distance deterrence ($\beta$). |
