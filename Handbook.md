...
>
> 3. **The Core Scientific Gap: Prediction vs. Behavioural Identifiability under Information Loss / Khoảng trống Khoa học Cốt lõi: Dự báo vs. Tính Định danh Hành vi dưới Mất mát Thông tin:** 
>    - **EN:** Contemporary mobility research—including SOTA AI baselines (Deep Gravity \citep{simini2021}, UGNN \citep{guo2025universal}, neuroGravity \citep{neurogravity2026}, TransGM \citep{transgm2026})—predominantly focuses on **predictive optimization** ($\min \mathcal{L}(T_{ij}^{obs}, \hat{T}_{ij})$ or generating flows for unseen target cities). However, predicting flows does not answer the fundamental scientific question: *what behavioural information remains statistically identifiable under aggregate observation loss?* The core novelty and scientific contribution of this work lie in shifting the research objective from predictive optimization to **probabilistic behavioural identifiability** under spatial information loss.
>    - **VI:** Các nghiên cứu di chuyển hiện đại—bao gồm cả các baseline AI SOTA (Deep Gravity \citep{simini2021}, UGNN \citep{guo2025universal}, neuroGravity \citep{neurogravity2026}, TransGM \citep{transgm2026})—chủ yếu tập trung vào **tối ưu hóa dự báo** ($\min \mathcal{L}(T_{ij}^{obs}, \hat{T}_{ij})$ hoặc sinh lưu lượng cho các đô thị mới). Tuy nhiên, việc dự báo lưu lượng không giải quyết câu hỏi khoa học nền tảng: *thông tin hành vi nào vẫn có thể định danh thống kê dưới sự mất mát quan sát tổng hợp?* Tính mới cốt lõi và đóng góp khoa học của công trình này nằm ở việc chuyển dịch mục tiêu từ tối ưu hóa dự báo sang **tính định danh hành vi xác suất** dưới sự mất mát thông tin không gian.
>
...
>      \[
>      T_{ij} = \underbrace{O_i \frac{A_j}{\sum_m A_m f(d_{im};\boldsymbol{\theta})}}_{\text{Urban Structure (Mobility Potential Field)}} \cdot \underbrace{f(d_{ij};\boldsymbol{\theta})}_{\text{Behaviour}}
>      \]
...
>      
>      The structural terms $(O_i, A_j)$ define the **Mobility Potential Field** comprising **Production Potential ($O_i$)** (origin trip emission capacity) and **Attraction Potential ($A_j$)** (destination opportunity density), which specify *where spatial opportunities exist*. In contrast, the deterrence function $f(d_{ij};\boldsymbol{\theta})$ describes **Behaviour of Spatial Interaction**, defined as the **collective distance sensitivity governing the utilization of spatial opportunities**. In this Handbook, we interpret this factorized form as establishing fundamentally distinct functional roles for urban structure and travel behaviour, forming the first foundational principle of PCSF-TIM. *(Notation Standard: Origin production potential is denoted $O_i$ and destination attraction potential is denoted $A_j$ throughout this Handbook).*
>    - **VI:** Mô hình tương tác Trọng lực phân rã tương tác không gian thành hai thành phần có vai trò chức năng riêng biệt:
>      \[
>      T_{ij} = \underbrace{O_i \frac{A_j}{\sum_m A_m f(d_{im};\boldsymbol{\theta})}}_{\text{Cấu trúc Đô thị (Mobility Potential Field)}} \cdot \underbrace{f(d_{ij};\boldsymbol{\theta})}_{\text{Hành vi}}
>      \]
>      Cấu trúc đô thị và Hành vi đóng vai trò bổ sung cho nhau chứ không tương đương trong tương tác không gian. Cấu trúc đô thị định nghĩa sự phân bố các cơ hội di chuyển thông qua tiềm năng phát sinh điểm đi và tiềm năng thu hút điểm đến, trong khi Hành vi quyết định cách người di chuyển khai thác các cơ hội đó bằng việc đánh đổi giữa cơ hội chuyến đi và chi phí khoảng cách. Do đó, luồng di chuyển quan sát được nảy sinh từ sự tương tác giữa một trường cơ hội và một cơ chế độ nhạy khoảng cách tập thể.
>
>      Các thuật ngữ cấu trúc $(O_i, A_j)$ định nghĩa **Trường Tiềm năng Di chuyển (Mobility Potential Field)** gồm **Tiềm năng Phát sinh ($O_i$)** và **Tiềm năng Thu hút ($A_j$)**, xác định *nơi các cơ hội không gian tồn tại*. Ngược lại, hàm cản trở $f(d_{ij};\boldsymbol{\theta})$ biểu diễn **Hành vi Tương tác Không gian**, được định nghĩa là **độ nhạy khoảng cách tập thể chi phối việc khai thác các cơ hội không gian**. Cách diễn giải này tạo thành nguyên lý nền tảng đầu tiên của PCSF-TIM. *(Quy chuẩn ký hiệu: Tiềm năng phát sinh chuyến đi điểm đi ký hiệu là $O_i$ và tiềm năng thu hút cơ hội điểm đến ký hiệu là $A_j$ trong xuyên suốt Handbook này).*
...
>      - This ensures that parameter identification evidence is derived exclusively from the empirical aggregate observation space under the Frequentist Likelihood Principle.
...
>      - Điều này đảm bảo rằng bằng chứng định danh tham số được rút ra hoàn toàn từ không gian quan sát tổng hợp thực nghiệm theo Nguyên lý Likelihood Frequentist.
>
...
| **Module Rationale** | Gravity is adopted not because it predicts best across all benchmarks, but because it provides an explicit scientific representation of the structure–behaviour decomposition required for behavioural identification \citep{wilson1971, erlander1990spatial, okelly2009spatial}. | Trọng lực không được lựa chọn vì nó luôn dự báo tốt nhất, mà vì nó cung cấp biểu diễn khoa học rõ ràng về sự phân rã cấu trúc - hành vi bắt buộc cho việc định danh hành vi \citep{wilson1971, erlander1990spatial, okelly2009spatial}. |
| **Mission** | Establish that the Gravity model provides an appropriate scientific representation for studying aggregate human mobility because it explicitly decomposes mobility flows into urban structure and collective behavioural response. This decomposition provides an interpretable and statistically identifiable foundation for behavioural inference, and motivates Gravity as the structural starting point for the remainder of the handbook. | Thiết lập rằng mô hình Trọng lực cung cấp biểu diễn khoa học phù hợp để nghiên cứu di chuyển con người tổng hợp vì nó phân rã một cách rõ ràng lưu lượng di chuyển thành cấu trúc đô thị và phản ứng hành vi tập thể. Sự phân rã này cung cấp một nền tảng có thể giải thích được và có thể định danh thống kê cho suy luận hành vi, và là cơ sở lý thuyết cho toàn bộ Handbook. |
...
>
> **EN:** The Gravity interaction model is adopted in this Handbook **not because it claims to be the highest-performing predictive benchmark model**, but because it provides an **explicit, transparent mathematical representation for decomposing Urban Spatial Structure ($O_i, A_j$) from Collective Travel Behaviour ($f(d_{ij};\theta)$)** \citep{wilson1971, erlander1990spatial, okelly2009spatial}:
> \[
...
| :--- | :--- | :--- | :--- |
| **A1. Primary contribution is explicit separation between structure and behaviour.**<br>*Đóng góp nền tảng là tách biệt rõ ràng giữa cấu trúc và hành vi.* | Establishes operational integration $R_S + R_B \longrightarrow \text{Spatial Interaction Process} \longrightarrow T_{ij}$ as theoretical core.<br>*Thiết lập sự tích hợp thao tác $R_S + R_B \longrightarrow \text{Tương tác Không gian} \longrightarrow T_{ij}$ làm lõi lý thuyết.* | • Zipf (1946) \citep{zipf1946}.<br>• Hansen (1959) \citep{hansen1959accessibility}.<br>• Wilson (1971) \citep{wilson1971}.<br>• Erlander & Stewart (1990) \citep{erlander1990spatial}.<br>• O'Kelly (2009) \citep{okelly2009spatial}.<br>• Lenormand et al. (2016) \citep{lenormand2016systematic}. | Gravity provides a clear decomposition to isolate structure from behaviour.<br>*Mô hình trọng lực cung cấp sự phân rã cần thiết để tách biệt cấu trúc đô thị khỏi phản ứng hành vi.* |
...
> **EN:** **Empirically,** individual mobility trajectories exhibit high spatio-temporal uniqueness: just four location-time points are sufficient to re-identify approximately 95% of individuals \citep{de2013unique}. Furthermore, as \citet{gallotti2024distorted} demonstrated, privacy-preserving matrix pruning distorts microscopic origin-destination flow matrices. In response, aggregate mobility products—including Meta's Movement Distribution Maps (MDM) \citep{MetaMovementDistributionMaps}—are becoming increasingly available across platforms and regions, providing privacy-preserving summaries of population travel behavior \citep{buckee2020thinking, oliver2020mobile, pappalardo2023analytical}.
>
...
> - **If YES (Hypothesis Supported):** Provides empirical evidence for a methodological finding that aggregate mobility products suffice for behavioural parameter estimation without requiring local flow surveys.
...
> Both outcomes hold scientific value for mobility science.
>
...
| **Core Scientific Gap** | **Existing spatial interaction literature predominantly optimizes predictive mobility models or downstream fitting metrics, leaving unaddressed the fundamental scientific question of what behavioural information remains statistically identifiable directly from aggregate observations under spatial information loss.** | **Văn liệu tương tác không gian hiện tại chủ yếu tập trung tối ưu hóa các mô hình dự báo hoặc các chỉ số đánh giá hạ nguồn, bỏ ngỏ câu hỏi khoa học nền tảng về việc thông tin hành vi nào vẫn có thể định danh thống kê trực tiếp từ quan sát tổng hợp dưới sự mất mát thông tin không gian.** |
...
>
> **Trong Handbook này (In this handbook),** chúng tôi nhấn mạnh rằng câu hỏi khoa học trung tâm chưa có lời giải không phải là cách dự báo di chuyển từ dữ liệu tổng hợp, mà là thông tin hành vi nào vẫn có thể định danh thống kê sau khi các quan sát di chuyển đã bị tổng hợp.
