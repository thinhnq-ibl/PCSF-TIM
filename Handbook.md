...
| **Core Research Gap** | **To the best of our knowledge, existing spatial interaction literature has not formulated a probabilistic inference framework that directly identifies behavioural distance-decay parameters from aggregate TLDs conditioned on independently specified urban structural exposure.** | **Theo hiểu biết của chúng tôi, văn liệu tương tác không gian hiện nay chưa công bố một khung suy luận xác suất cho phép định danh trực tiếp các tham số suy giảm khoảng cách từ TLD tổng hợp khi điều kiện hóa trên mức độ tiếp xúc cấu trúc đô thị được xác định độc lập.** |
