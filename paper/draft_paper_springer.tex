...
\abstract{Accurate origin--destination (OD) flow estimation is fundamental to transportation planning, accessibility analysis, and urban management. Existing gravity-model calibration approaches rely on observed OD matrices or individual mobility trajectories, leaving it unclear whether aggregate mobility data preserve sufficient information to recover the latent city-specific distance-decay parameter governing spatial interactions. This study investigates whether aggregate mobility data can recover the latent city-specific distance-decay parameter required for gravity-model calibration without observed OD matrices. We first derive aggregate trip-length distributions from privacy-preserving sources such as Meta Movement Distribution Maps. Next, the latent distance-decay parameter is recovered and used to calibrate a gravity model. The calibrated model is then applied to estimate OD flows across 50 US metropolitan areas. The recovered distance-decay parameters and the resulting gravity models are evaluated against locally calibrated references. Results show that the recovered distance-decay parameters closely match locally calibrated references and yield downstream flow predictions statistically equivalent to locally-calibrated models within a 0.01 Common Part of Commuters (CPC) margin. A Shapley analysis further reveals that distance decay accounts for 85.8\% of the total predictive gain. These findings indicate that aggregate mobility data preserve sufficient information to recover latent city-specific distance-decay parameters without requiring observed OD matrices. Building on this insight, this work introduces a zero-shot gravity-model calibration framework that enables transferable OD estimation for cities lacking observed OD matrices, with privacy-preserving aggregate mobility serving as a practical and scalable data source.}
...
Accurate origin--destination (OD) flow prediction is fundamental to transportation planning, infrastructure development, and accessibility analysis. Reliable OD information supports travel demand forecasting and urban policy analysis, making it a central problem in transportation science. To model these spatial interactions, classical approaches such as gravity models~\citep{wilson1971,tanner1961} rely on a distance-decay parameter ($\beta$) to capture the behavioral friction of travel distance. More recent learning-based frameworks, including DeepGravity and graph neural networks~\citep{simini2021,hu2020mpgcn}, similarly depend on learning spatial friction representations from training data.
...
The rest of this paper is organized as follows. Section~\ref{sec:related_work} reviews related work. Section~\ref{sec:problem_setting} formalizes the problem setting. Section~\ref{sec:methodology} presents the behavioral inference framework. Section~\ref{sec:results} reports the experimental validation. Section~\ref{sec:discussion} discusses the implications of our findings, and Section~\ref{sec:threats_to_validity} addresses threats to validity.
...
This study demonstrates that aggregate mobility data preserve sufficient information to recover latent city-specific distance-decay functions, establishing a robust foundation for survey-free behavioral inference and enabling zero-shot gravity-model calibration across urban systems.
