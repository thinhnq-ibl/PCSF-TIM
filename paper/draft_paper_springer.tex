...
\subsection{Observation Challenge}
...
\subsection{Opportunity from Aggregate Mobility}
...
\begin{quote}
\emph{Aggregate travel-distance distributions preserve sufficient statistical information to identify latent city-specific distance-deterrence parameters.}
\end{quote}
...
\begin{enumerate}
    \item \textbf{Prediction 1 (Statistical Identifiability):} Aggregate travel-distance observations induce a statistically valid multinomial likelihood for identifying latent distance-deterrence parameters.
    \item \textbf{Prediction 2 (Parameter Consistency):} Building on Prediction~1, distance-deterrence parameters identified from aggregate travel-distance distributions are statistically consistent with reference parameters calibrated from complete origin--destination interactions.
    \item \textbf{Prediction 3 (Predictive Consequence):} Building on Prediction~2, distance-deterrence parameters identified from aggregate observations preserve the downstream predictive performance of gravity-model calibration in unobserved target cities.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Layer 1 — Scientific Discovery (Core Contribution):} We demonstrate that aggregate travel-distance distributions preserve sufficient statistical information to identify latent city-specific distance-deterrence parameters without observing origin--destination interactions.
    \item \textbf{Layer 2 — Methodological Contribution:} Building on this discovery, we establish a probabilistic parameter-identification framework to infer latent distance-deterrence parameters directly from aggregate travel-distance observations and integrate them into gravity-model calibration.
    \item \textbf{Layer 3 — Practical Contribution:} Building on this framework, we enable transferable, survey-free, and privacy-preserving gravity-model calibration for data-scarce cities lacking target-city OD observations.
\end{enumerate}

\subsection{Paper Organization}
The remainder of this paper is organized as follows: Section~\ref{sec:related_work} reviews related work on spatial interaction modeling, parameter identification, aggregate mobility representations, and survey-free prediction. Section~\ref{sec:problem_formulation} formulates gravity calibration as a probabilistic parameter-identification problem under aggregate observations. Section~\ref{sec:probabilistic_framework} presents the proposed probabilistic parameter-identification framework. Section~\ref{sec:results} empirically evaluates the three predictions across 50 US metropolitan areas. Section~\ref{sec:discussion} discusses scientific implications, scope, limitations, and future directions. Section~\ref{sec:conclusion} concludes the paper.
...
\subsection{Spatial Interaction Modeling under Limited Observations}
Spatial interaction modeling is essential for travel demand forecasting, accessibility assessment, and urban policy evaluation. Classical spatial interaction frameworks estimate mobility through gravity models \citep{wilson1971} or radiation formulations \citep{Simini2012universal}, while recent neural approaches leverage spatial representations learned from mobility data \citep{simini2021,transgm2026}. Despite structural differences, existing models overwhelmingly rely on observed OD flows or trajectory records to calibrate distance-deterrence parameters or train spatial interaction functions \citep{lenormand2016systematic}.

\subsection{Distance-Deterrence Parameter Identification}
Distance-deterrence functions quantify how travel impedance reduces interaction probability \citep{tanner1961,fotheringham1989spatial,sen1995gravity}. Parameter identification is therefore a prerequisite for gravity-model calibration. Existing calibration methods assume that complete OD matrices are available to fit deterrence parameters via maximum likelihood or least squares \citep{flowerdew1982method,rubioherrero2023sparse}. Methodological research has focused primarily on fitting techniques given complete OD data, leaving unaddressed whether parameters can be identified without observed OD matrices.

\subsection{Aggregate Mobility as a Statistical Representation}
Privacy-preserving aggregate mobility products summarize population movement into trip-length distributions while discarding location identities \citep{oliver2020mobile,buckee2020aggregated,pappalardo2023analytical}. While prior work uses aggregate summaries for descriptive visualization or epidemic tracking, aggregate travel-distance distributions preserve distributional information governing collective movement patterns. Whether this representation retains sufficient information for parameter identification remains largely unexplored.

\subsection{Survey-Free Mobility Modeling}
Recent transferable mobility models (e.g., DeepGravity \citep{simini2021}, neuroGravity \citep{neurogravity2026}, TransGM \citep{transgm2026}, GODDAG \citep{Rong2023goddag}) reduce data requirements in target domains by learning mobility representations in source cities. However, these models assume that spatial interaction parameters or deep representations are learned from observed OD interactions before transfer. None directly identify distance-deterrence parameters from aggregate observations alone.

\subsection{Research Gap: From Observation to Parameter Identification}
While existing research focuses on predicting flows under data constraints, a fundamental scientific question remains unresolved: \emph{Can aggregate travel-distance distributions themselves provide sufficient statistical information for identifying latent distance-deterrence parameters?} This paper addresses this gap by formulating gravity calibration as a probabilistic parameter-identification problem under aggregate observations.

\section{Problem Formulation}\label{sec:problem_formulation}

Rather than estimating origin--destination (OD) flows directly, this study formulates gravity calibration as the problem of identifying latent distance-deterrence parameters $\theta$ from aggregate travel-distance observations. 

The primary objective is to identify city-specific distance-deterrence parameters $\theta = (\alpha, \beta) \in \Theta$ from a spatial representation layer $\mathbf{b} = (b_1, \dots, b_K)^\top \in \Delta^{K-1}$ summarizing population travel-distance distributions. The secondary objective is to evaluate whether the identified parameters $\hat{\theta}$ enable survey-free gravity-model calibration to predict unobserved target-city OD matrices $\hat{\mathbf{T}}$.

\section{Probabilistic Parameter-Identification Framework}\label{sec:probabilistic_framework}

\subsection{Layer 1 — Physical Spatial Interaction Model}
We consider a spatial interaction system comprising $N_{\text{zones}}$ geographic zones. The expected physical flow $T_{ij}$ from origin zone $i$ to destination zone $j$ under a singly constrained gravity specification is expressed in separable form:
...
\subsection{Layer 2 — Observation Model}
Let $\mathcal{P}: \mathbb{R}^{N_{\text{zones}} \times N_{\text{zones}}} \to \mathbb{R}^K$ denote a linear observation operator projecting origin--destination interactions onto a $K$-bin aggregate distance distribution:
...
\subsection{Layer 3 — Probabilistic Observation Model}
The model-predicted probability $\pi_k(\theta)$ of a trip falling within distance bin $k$ under parameter vector $\theta$ is derived by marginalizing joint trip probabilities across zonal pairs:
...
\end{equation}
where $E_k = \sum_{i,j: d_{ij} \in \text{bin } k} O_i A_j$ is the geometric spatial exposure of bin $k$, $d_k$ is the representative midpoint distance of bin $k$, and $f(d; \theta) = (d + \delta)^{-\alpha} \exp(-\beta d)$ denotes the Tanner deterrence function with adaptive offset $\delta = 0.5 \bar{R}_{\mathrm{zone}}$.
...
\begin{equation}\label{eq:multinomial_dist}
N \mathbf{b} \sim \mathrm{Multinomial}(N, \boldsymbol{\pi}(\theta)).
\end{equation}

\subsection{Layer 4 — Likelihood & Statistical Parameter Identification}
The log-likelihood of observing aggregate distribution $\mathbf{b}$ given parameter vector $\theta$ is:
...
\subsection{Origin Production & Destination Attraction Estimation}
Origin trip production $\hat{O}_i$ is estimated independently of distance-deterrence identification using spatial machine learning models $g(\mathbf{x}_i)$ trained on open demographic and infrastructure features. Destination attraction $\hat{A}_j$ is specified independently using composite spatial indicators $\hat{A}_j = \frac{1}{2} [\mathrm{minmax}(P_j) + \mathrm{minmax}(\mathrm{POI}_j)]$, preserving the decoupled structure of the gravity model.

\subsection{Layer 5 — Downstream Flow Prediction}
Once latent deterrence parameters $\hat{\theta} = (\hat{\alpha}, \hat{\beta})$ are identified via Eq.~(\ref{eq:optimization_obj}), downstream spatial interaction flows $\hat{T}_{ij}$ are generated by combining the identified parameters with independent production and attraction estimates:
...
\begin{enumerate}
    \item \textbf{Property 1 (Statistical Identifiability):} Latent distance-deterrence parameters can be statistically identified under the assumed observation model provided the observation vector preserves $K-1 \ge p$ degrees of freedom.
    \item \textbf{Property 2 (Observation Efficiency):} Complete origin--destination flow observations are unnecessary for identifying city-specific deterrence parameters.
    \item \textbf{Property 3 (Decomposability):} Distance deterrence, origin production, and destination attraction are estimated independently.
    \item \textbf{Property 4 (Transferability):} The identified parameters enable survey-free gravity calibration in unseen cities without local target-city OD surveys.
\end{enumerate}
...
\subsection{Prediction 1 — Statistical Identifiability}\label{sec:pred1}
\textbf{Prediction 1:} \emph{Aggregate travel-distance observations induce a statistically valid likelihood for identifying latent distance-deterrence parameters.}

\textbf{Evidence:} The probabilistic observation model (Eq.~\ref{eq:multinomial_loglik}) establishes a multinomial log-likelihood over aggregate travel-distance bins. As shown across synthetic and real Meta MDM telemetry, cross-entropy minimization consistently identifies latent Tanner parameters $\theta = (\alpha, \beta)$ without observing origin--destination pairs, confirming statistical identifiability across all target metropolitan areas.

\subsection{Prediction 2 — Parameter Consistency}\label{sec:pred2}
\textbf{Prediction 2:} \emph{Parameters identified from aggregate travel-distance distributions are statistically consistent with those calibrated from complete OD interactions.}

\textbf{Evidence:} We compare parameters recovered from aggregate distance distributions ($\theta_{\text{hist}}$) against reference parameters calibrated on complete LODES OD matrices ($\theta_{\text{OD}}$) across 25 target cities. As shown in Table~\ref{tab:decay_recovery_merged}, recovered power-law exponent $\alpha$ exhibits strong structural consistency with reference OD parameters ($r = 0.909$, median relative error $1.7\%$, 100\% of cities within $\pm 10\%$). Exponential decay rate $\beta$ shows a tight absolute margin of error (MAE $= 0.0026$). Figure~\ref{fig:tanner_comparison} demonstrates that aggregate-identified deterrence curves closely match full OD calibration.
...
\subsection{Prediction 3 — Predictive Consequence}\label{sec:pred3}
\textbf{Prediction 3:} \emph{Parameters identified from aggregate observations preserve the downstream predictive performance of gravity-model calibration.}

\textbf{Evidence:} We evaluate downstream flow predictions generated using aggregate-identified parameters against baseline models across 25 held-out target cities (Table~\ref{tab:multibaseline_comparison}). Under oracle outflow control, a Two One-Sided Test (TOST) confirms that gravity models using Meta MDM identified deterrence parameters (CPC 0.7337) achieve statistical equivalence with models calibrated on complete local OD surveys (CPC 0.7382) within an equivalence margin of $\Delta_{\text{eq}} = 0.01$ CPC ($p = 0.00326$). Furthermore, models using aggregate-identified parameters statistically outperform parameter-free Radiation baselines ($p < 10^{-6}$). Figure~\ref{fig:scatter_flow} illustrates predicted vs. observed flow agreement.
...
\begin{enumerate}
    \item \textbf{Finding 1 (Statistical Identifiability):} Aggregate travel-distance distributions preserve sufficient statistical information to identify latent distance-deterrence parameters without observing OD interactions.
    \item \textbf{Finding 2 (Parameter Consistency):} Parameters identified from aggregate observations are statistically consistent with those calibrated from complete OD interactions.
    \item \textbf{Finding 3 (Predictive Consequence):} The identified parameters preserve the predictive consequences of gravity-model calibration, enabling competitive survey-free OD estimation.
\end{enumerate}

\section{Discussion}\label{sec:discussion}

\subsection{Scientific Implications: Statistical Information in Aggregate Observations}\label{sec:paradigm_shift}
...
\begin{enumerate}
    \item \textbf{Scientific Scope:} The study focuses on identifying distance-deterrence parameters within gravity-model formulations. Individual psychological travel choices remain outside the current probabilistic framework.
    \item \textbf{Methodological Scope:} The formulation is established under singly constrained gravity assumptions. Extension to alternative spatial interaction models remains future work.
    \item \textbf{Experimental Scope:} Validation is conducted across 50 metropolitan areas within one national context; broader cross-national evaluation is warranted.
    \item \textbf{Observation Scope:} The framework considers one-dimensional aggregate travel-distance distributions; multi-dimensional aggregate representations may yield additional information.
\end{enumerate}

\subsection{Future Directions}
Promising future research directions include:
\begin{itemize}
    \item Formulating higher-dimensional aggregate mobility representations.
    \item Exploring alternative distance-deterrence functional forms.
    \item Developing general probabilistic observation models for non-gravity interaction systems.
    \item Integrating probabilistic parameter identification with physics-informed neural architectures.
    \item Expanding empirical validation across heterogeneous international mobility systems.
\end{itemize}
...
Building on this discovery, the proposed probabilistic calibration framework identifies latent distance-deterrence parameters directly from aggregate travel-distance observations and integrates them with independently estimated production and attraction components for survey-free gravity calibration. Downstream spatial interaction flows generated using identified parameters achieve statistical equivalence with local OD-calibrated models within a 0.01 CPC margin, with distance friction accounting for 85.8\% of total predictive gain.
