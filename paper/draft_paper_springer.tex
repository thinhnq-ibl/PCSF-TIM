...
\abstract{Accurate origin--destination (OD) flow prediction is essential for transportation planning, accessibility analysis, and urban management. Existing approaches typically require observed OD matrices or individual mobility trajectories, limiting their applicability in data-scarce and privacy-sensitive settings. This study investigates whether aggregate mobility data can recover city-specific distance-decay functions for gravity-model calibration. We first derive aggregate trip-length distributions from privacy-preserving sources such as Meta Movement Distribution Maps. Next, we estimate the underlying distance-decay function and calibrate a gravity model. Finally, the calibrated model generates OD flow estimates using globally available spatial covariates. The proposed framework is evaluated across 50 US metropolitan areas and compared with representative gravity-model calibration baselines. Results show that the recovered distance-decay functions closely match reference patterns and yield downstream flow predictions statistically equivalent to locally-calibrated models within a 0.01 Common Part of Commuters (CPC) margin. A Shapley analysis further reveals that distance decay accounts for 85.8\% of the total predictive gain. These findings demonstrate that privacy-preserving aggregate mobility products contain sufficient information to calibrate gravity models without direct OD observations. This work provides a transferable and privacy-preserving framework for OD flow estimation, expanding its applicability to data-scarce urban environments.}
