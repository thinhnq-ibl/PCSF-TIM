...
\begin{enumerate}
  \item Can aggregate trip-length distributions reliably recover city-specific distance-decay functions?
  \item Is distance decay the dominant transferable factor in zero-shot gravity modeling?
  \item To what extent can distance decay recovered from aggregate trip-length distributions support zero-shot transfer performance?
\end{enumerate}

Our empirical investigation across 50 US metropolitan areas evaluates these assumptions. The results indicate that distance decay represents a dominant transferable factor across metropolitan areas, and that projecting spatial interactions onto the TLD provides a summary representation sufficient to identify this component. Although the proposed model remains below the DeepGravity benchmark (average CPC 0.6860 vs. 0.7529), the results show that aggregate mobility distributions contain sufficient information to recover the dominant transferable component of spatial interaction. Consequently, survey-free calibration from aggregate data can achieve competitive performance without requiring access to individual-level mobility trajectories. The rest of this paper is organized around the three research questions detailed above.
...
Table~\ref{tab:literature_comparison} compares how existing model classes address the core challenge of zero-shot cross-city transfer. The comparison reveals two core research gaps: the lack of an aggregate-based decay calibration method for zero-shot transfer, and the absence of a quantitative analysis regarding the impact of distance decay on zero-shot transfer. In addition, we explore whether urban morphology may influence the effectiveness of aggregate calibration. These gaps and exploratory questions map directly to the research questions evaluated throughout this study.
...
\subsection{Can aggregate trip-length distributions reliably recover city-specific distance-decay functions?}\label{sec:rq1}
...
To investigate how the granularity of distance binning affects model performance, we sweep the number of distance bins $K$ across $\{3, 5, 10, 20\}$. The results demonstrate that downstream predictive accuracy (measured in CPC) stabilizes at $K \ge 5$, yielding a mean CPC of $0.7174$ for $K=3$, $0.7346$ for $K=5$, $0.7394$ for $K=10$, and $0.7403$ for $K=20$. Detailed comparisons and the corresponding sensitivity heatmap are provided in Appendix~A.2 (Fig.~\ref{fig:layer1_heatmap}).

\subsection{Is distance decay the dominant transferable factor in zero-shot gravity modeling?}\label{sec:rq2}
To address the second research question, we first evaluate the origin production component ($O_i$) in isolation to validate the predictive quality of our GBDT outflow models, and then implement a game-theoretic Shapley value analysis to quantify the relative contributions of each individual model component to the overall predictive accuracy.
...
\subsection{To what extent can distance decay recovered from aggregate trip-length distributions support zero-shot transfer performance?}\label{sec:rq3}
...
Preliminary patterns suggest that the physics-constrained gravity formulation achieves its highest predictive performance in Coastal environments (mean CPC of 0.714). The relative performance gap between the proposed gravity model and the neural DeepGravity baseline narrows to 5.8\% in Coastal cities, compared to a larger 10.3\% gap in Dense / Transit environments. The observations indicate that physical geographical barriers may restrict alternative routes and amplify the explanatory role of distance decay in commuting flows. We note, however, that these morphology-specific findings should be interpreted cautiously due to several key statistical limitations: the overall sample size of target cities is small ($N=25$), the morphology groups are highly unbalanced (e.g., only 3 Coastal cities and 5 Dense / Transit cities), and the resulting statistical power is limited.
...
We demonstrate that target-city aggregate TLDs contain sufficient signal to recover distance-decay parameters with high fidelity, achieving downstream prediction performance statistically equivalent to models calibrated directly on target-city individual-level OD flows. Furthermore, a game-theoretic Shapley analysis shows that distance decay dominates spatial interaction predictive capacity, accounting for 85.8\% of the total CPC gain. Although localized calibration from real-world telemetry does not outperform a robust national decay fallback, it does not require historical survey data from source cities, thereby enabling zero-shot deployment in data-scarce regions where such national priors cannot be constructed.
