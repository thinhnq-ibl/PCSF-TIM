...
\subsection{Relationship Between Cross-Entropy Minimization and Maximum Likelihood Estimation}\label{sec:mle_relationship}

To establish the statistical foundations of the proposed behavioral recovery module, we clarify the mathematical connection between cross-entropy minimization over aggregate travel fractions and classical maximum likelihood estimation (MLE) over disaggregate trip observations.

Let $\mathbf{y} = (y_1, y_2, \dots, y_K)^\top$ represent an empirical vector of trip counts across $K$ distance intervals, with total sample size $N = \sum_{k=1}^K y_k$. When individual mobility traces are unobservable due to privacy restrictions, telemetry products (such as Meta Movement Distribution Maps) release normalized travel fractions $\mathbf{b} = (b_1, b_2, \dots, b_K)^\top$, where $b_k = y_k / N$ and $\sum_{k=1}^K b_k = 1.0$.

Under a multinomial sampling assumption, the log-likelihood of observing count vector $\mathbf{y}$ given exposure-corrected model probabilities $\boldsymbol{\pi}(\theta)$ is
\begin{equation}\label{eq:multinomial_loglik}
\log \mathcal{L}(\theta; \mathbf{y}) = \log \left( \frac{N!}{\prod_{k=1}^K y_k!} \right) + \sum_{k=1}^K y_k \log \pi_k(\theta) = C + N \sum_{k=1}^K b_k \log \pi_k(\theta),
\end{equation}
where $C$ is a constant independent of $\theta$. Rearranging Eq.~(\ref{eq:multinomial_loglik}) yields
\begin{equation}\label{eq:mle_cross_entropy_relation}
\log \mathcal{L}(\theta; \mathbf{y}) = C - N \cdot H(\mathbf{b}, \boldsymbol{\pi}_\theta) = C - N \left[ H(\mathbf{b}) + D_{\mathrm{KL}}(\mathbf{b} \parallel \boldsymbol{\pi}_\theta) \right],
\end{equation}
where $H(\mathbf{b}, \boldsymbol{\pi}_\theta) = -\sum b_k \log \pi_k(\theta)$ is the cross-entropy, $H(\mathbf{b})$ is the empirical entropy of the aggregate telemetry, and $D_{\mathrm{KL}}(\mathbf{b} \parallel \boldsymbol{\pi}_\theta)$ is the Kullback--Leibler (KL) divergence (relative entropy).
...
\nabla_\theta \log \mathcal{L}(\theta; \mathbf{y}) = -N \cdot \nabla_\theta H(\mathbf{b}, \boldsymbol{\pi}_\theta) = 0 \iff \nabla_\theta H(\mathbf{b}, \boldsymbol{\pi}_\hat{\theta}) = 0 \quad (\text{for } N > 0).
\end{equation}

Equation~(\ref{eq:score_invariance}) shows that the recovered parameters are invariant to the total sample size ($N$). Consequently, optimizing normalized aggregate fractions yields the same optimum as multinomial maximum likelihood whenever multinomial counts are available, while remaining directly applicable to privacy-preserving aggregate mobility products that release only normalized distributions. Therefore, cross-entropy minimization can be viewed as the normalized counterpart of multinomial maximum likelihood estimation. This formulation preserves the same optimal parameter estimates while naturally supporting privacy-preserving aggregate mobility products that provide normalized trip-length distributions instead of raw trip counts.
