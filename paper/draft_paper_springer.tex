...
Gravity-based spatial interaction models have attracted sustained interest across transport geography and urban science because of their structural decomposability and physical interpretability \citep{wilson1971,tanner1961,martinez2013distance}. Their application spans classical trip distribution modeling \citep{lenormand2016systematic} to modern hybrid formulations that integrate machine learning components to improve predictive accuracy \citep{gravigbm2024}. Despite this diversity, all gravity-based approaches share a common dependency: calibrating their distance-decay parameters requires access to local OD observations, which are costly to collect and often unavailable in data-scarce regions.

Meanwhile, aggregate mobility products---such as population-level trip-length histograms---are becoming increasingly available across platforms and regions. Whether these aggregate summaries preserve sufficient structural information to substitute for OD observations in gravity calibration remains an open question. More broadly, while deep learning methods achieve high predictive accuracy, they too require large-scale individual OD records for training \citep{simini2021}, limiting applicability under strict privacy constraints. This highlights a critical unresolved question: \textbf{it remains unknown whether aggregate trip-length distributions preserve sufficient information to identify the dominant predictive component of gravity-based spatial interaction.}

In this paper, we address this gap through \textbf{SDF-ZTF} (Separable Decomposed Formulation for Zero-Shot Transferable Flows). Previous work has established that calibration-free mobility modeling is feasible \citep{yang2014limits} and that distance-decay functions are central determinants of spatial interaction \citep{verma2025determinants}, yet it remains unknown whether aggregate trip-length distributions alone can recover these functions with sufficient fidelity for zero-shot flow prediction. We investigate three sequential research questions that together test the feasibility, dominance, and practical utility of distance-decay recovery from aggregate data.
...
In contrast, this study investigates a fundamentally different question: can aggregate trip-length distributions alone preserve sufficient information to recover the dominant distance-decay mechanism required for gravity-based OD generation? This formulates an \emph{inverse mobility problem}: whether aggregate TLDs contain sufficient information to identify city-specific distance-decay parameters without any OD supervision. Previous work has highlighted the limits of predictability when calibration data are absent \citep{yang2014limits}, and has shown that distance-decay functions vary substantially across urban contexts \citep{verma2025determinants}---yet neither line of work has demonstrated that aggregate TLDs alone can serve as the primary calibration signal for recovering the dominant distance-friction mechanism. To the best of our knowledge, existing literature has not demonstrated a practical workflow in which aggregate TLDs serve as the primary information source for recovering city-specific distance-decay parameters and then generating OD flows under strict zero-shot conditions (no target-city OD observations, no trajectory supervision, and no target-domain adaptation). Accordingly, this work shifts the focus of zero-shot mobility modeling from learning OD flows to identifying the dominant transferable mechanism underlying those flows.
...
Gravity-Informed Neural Models \citep{rong2023odgn,zhu2026gravitypanel,zhao2025truckgravity} & Partial & Limited & Yes & Medium & Partial \\
