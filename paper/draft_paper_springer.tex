...
In this paper, we address this gap through \textbf{SDF-ZTF} (Separable Decomposed Formulation for Zero-Shot Transferable Flows). Building on the analytic calibration approach of Yang et al. (2014) \citep{yang2014limits} and the importance of distance-decay highlighted by Verma \& Ukkusuri (2025) \citep{verma2025determinants}, we investigate three sequential questions: (i) can aggregate trip-length distributions provide sufficient calibration information to recover city-specific distance-decay parameters? (ii) to what extent does the distance-decay component dominate predictive performance within a gravity-based framework? (iii) can recovered parameters enable competitive survey-free deployment without individual OD data? Under the primary \textbf{OD-free calibration setting}, the model recovers distance-decay parameters from target-city aggregate TLDs (e.g., TLDs derived from Meta Movement Distribution Maps). In both settings, zero-shot flow generation is driven by open spatial covariates, with OpenStreetMap-derived features as the primary city-level input for production and attraction estimation. In the \textbf{pure zero-shot fallback}, the model uses a national decay prior derived from source cities. We show that aggregate trip-length histograms provide sufficient calibration information to recover city-specific distance-decay parameters with high fidelity. Furthermore, our game-theoretic Shapley analysis shows that distance decay accounts for the majority of predictive gain in this framework.
