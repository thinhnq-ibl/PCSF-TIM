...
\subsection{The Practical Problem: OD Data Are Costly and Often Unavailable}

Predicting origin-destination (OD) travel flows is essential for urban transport planning, infrastructure design, and mobility policy evaluation \citep{barbosa2018human,gonzalez2008understanding,song2010limits}. Yet acquiring the OD data required to calibrate predictive models remains a fundamental bottleneck. Traditional travel surveys are expensive to conduct, require sustained institutional investment, and cover only a fraction of global cities \citep{gonzalez2008understanding}. Proprietary cellular or GPS records, while richer, are restricted by commercial licensing agreements and strict privacy regulations \citep{song2010limits}. As a result, the majority of cities worldwide lack any reliable OD survey, and the problem of \emph{survey-free flow prediction}---predicting commuting flows in a target city with no locally observed origin-destination pairs---remains largely unsolved.

\subsection{Existing Solutions and Their Shared Dependency}

Two broad families of models have been proposed. Classical gravity-based spatial interaction models \citep{wilson1971,tanner1961} and the radiation model \citep{Simini2012universal} are interpretable and physically motivated, but all gravity formulations require local OD surveys to calibrate their distance-decay parameters \citep{martinez2013distance,lenormand2016systematic}. Deep and graph learning approaches---including DeepGravity, MPGCN, and GODDAG \citep{simini2021,hu2020mpgcn,Rong2023goddag}---achieve high predictive accuracy but learn OD mappings from supervised historical flow matrices, imposing the same data dependency under a different guise. Transfer learning frameworks such as neuroGravity \citep{neurogravity2026} and TransGM \citep{transgm2026} reduce but do not eliminate this dependency: they still require either a pretrained source-city OD model or some degree of target-city structural data for adaptation. Consequently, no existing approach can calibrate a gravity model with competitive accuracy using only aggregated, privacy-safe mobility summaries.

\subsection{A New Opportunity: Aggregate Mobility Products}

At the same time, aggregate mobility products---including population-level trip-length distributions (TLDs) and platform-derived movement summaries such as Meta's Movement Distribution Maps (MDM) \citep{MetaMovementDistributionMaps}---are becoming increasingly available across platforms and regions, providing privacy-preserving summaries of population travel behavior. Unlike OD matrices, these products disclose only aggregated travel-distance histograms, making them substantially more accessible in data-scarce or privacy-restricted regions. Whether such aggregate summaries contain sufficient information to replace OD observations in the calibration of gravity models is, however, an open question.

More specifically, within the gravity framework, distance decay governs how interaction strength decreases with separation distance and has long been recognized as a central mechanism of spatial interaction \citep{tanner1961,verma2025determinants}. It remains unknown whether city-specific distance-decay functions can be recovered from aggregate trip-length distributions \emph{without observing any OD flows}. Prior work has demonstrated that aggregate statistics such as median travel time can calibrate single-parameter spatial interaction models \citep{merlin2020medians}, suggesting that aggregate representations may carry sufficient geometric information; however, whether this extends to the two-parameter Tanner deterrence function---which requires jointly estimating power-law and exponential components while correcting for zone geometry and attraction structure---remains uncharacterized.

\subsection{Research Questions}

In this paper, we address this gap through \textbf{SDF-ZTF} (Separable Decomposed Formulation for Zero-Shot Transferable Flows), which formulates gravity calibration as an \emph{inverse problem}: recovering city-specific distance-decay parameters from aggregate TLDs without any OD supervision. We structure our investigation around three research questions that form a causal chain from recoverability to information content to practical utility:
...
Our empirical investigation across 50 US metropolitan areas yields the following contributions:
\begin{enumerate}
  ...
  \item We develop a survey-free calibration pipeline (SDF-ZTF) that combines aggregate mobility summaries and open spatial data to achieve downstream performance statistically equivalent to locally-calibrated baselines, providing an interpretable and privacy-preserving pathway for zero-shot urban flow prediction.
\end{enumerate}
...
We model the distance-decay component using the Tanner multiplicative deterrence function \citep{tanner1961,feldman2012alternative}, which combines power-law and exponential terms and has been shown to outperform single-family alternatives across diverse urban settings \citep{verma2025determinants}:
