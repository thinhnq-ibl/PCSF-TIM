...
| **Module 4. Urban Structure Representation** | **How should Urban Structure be represented for spatial interaction modelling?** | Trace the evolution of Urban Structure representations from handcrafted census/POI variables to learned spatial representations operationalized as Mobility Potential Fields. | Urban Structure representation evolves from handcrafted variables to learned spatial representations operationalized as transferable potential fields (**Paper 2**). |
...
| **Module 6. Learning Representations for Spatial Interaction** | **How do learning objectives and strategies shape learned representations and their ability to generalize?** | Trace the methodological chain: **Learning Objective $\to$ Learning Strategy $\to$ Representation $\to$ Encoded Information $\to$ Generalization.** | Establish a methodological framework linking learning objectives and strategies to representation design, encoded spatial information, and cross-spatial generalization. |
...
[Decomposition Principle → two parallel pathways (Gap I → RQ1 → Paper 1; Gap II → RQ2 → Paper 2) → Joint Integration (RQ3) → Validation → Synthesis]
```
...
* **Claim B.3:** Modern platforms release aggregate mobility products—such as Meta Movement Distribution Maps (MDM)—which provide aggregated travel statistics that reduce disclosure granularity relative to disaggregated mobility traces.
...
| **C2. Information Hierarchy of Mobility** | How can mobility observations be systematically structured? | Mobility observations form a hierarchy of increasingly aggregated observation representations, each preserving different subsets of spatial and statistical information: Trajectories $\rightarrow$ OD Matrices $\rightarrow$ Distance Distributions (TLDs) $\rightarrow$ Summary Statistics. | de Montjoye, Y. A., Hidalgo, C. A., Verleysen, M., & Blondel, V. D. (2013). Unique in the crowd: The privacy bounds of human mobility. *Scientific Reports*, 3, 1376; Song et al. (2010); Barbosa et al. (2018). | Maps the transition from high-entropy, privacy-sensitive individual paths to compressed, anonymous population-level statistics. |
...
| **C6. Identification Sufficiency Principle (Transition to Layer D)** | What determines whether a compressed observation is scientifically sufficient for parameter identification? | Identification Sufficiency: An aggregate mobility observation is considered sufficient for parameter identification when, under a specified probabilistic spatial-interaction model and observation process, it contains enough statistical information to identify the target parameter with empirically validated consistency against an appropriate reference estimator. | *(Proposed in this thesis; grounded in information theory and motivated by Merlin's 2020 median-based estimator)* | Conceptual bridge to Layer D (The Gravity Principle and Scientific Mechanisms). |
...
* **Claim C.4 (Identification Sufficiency Principle):** Information reduction through aggregation does not automatically imply inferential insufficiency:
$$\text{Aggregation} \longrightarrow \text{Information reduction}$$
$$\text{Information reduction} \nRightarrow \text{Identification failure}$$
$$\text{Task-specific sufficiency} \longrightarrow \boldsymbol{\theta} \text{ identification}$$
An aggregate observation is sufficient for parameter identification when, under a specified probabilistic spatial-interaction model and observation process, it contains enough task-relevant statistical information to identify the target parameter $\boldsymbol{\theta}$ with empirically validated consistency against an appropriate reference estimator.
* **Claim C.5 (Resolution Aggregation Robustness):** Aggregating from a primary 20-bin TLD down to a coarse 3-bin TLD (simulating Meta MDM) tests whether the coarser observation retains sufficient task-relevant information for the behavioural identification task ($\hat{\boldsymbol{\theta}}_{20} \approx \hat{\boldsymbol{\theta}}_3$), rather than asserting full information equivalence between resolutions.
...
* **Proposed View:** TLD represents an aggregate, accessible observation space that contains sufficient statistical information for identifying the distance-decay parameter $\boldsymbol{\theta}$ under the specified spatial-interaction model.
...
| **D5. Scientific Interpretation of Gravity** | Is Gravity merely a predictive curve-fitting tool? | Gravity provides a mechanism-oriented modelling language for spatial interaction; empirical fit evaluates consistency with the proposed analytical decomposition under the specified model, rather than causal proof of real-world mechanisms. | Wilson, A. G. (1971); O'Kelly, M. E. (2009); Barbosa, H., et al. (2018). | Presents gravity as an entropy-maximizing model of spatial allocation under macro-state constraints, providing a mechanism-oriented framework rather than a black-box empirical regression. |
...
* **Parameter Identification & Shapley Attribution Evidence:** Game-theoretic Shapley value analysis across 50 US metropolitan areas demonstrates that, within the tested spatial-interaction decomposition, the distance-deterrence component $f(d)$ contributes 85.8% of the total CPC gain over the uniform-flow baseline ($\Delta\text{CPC} = 0.2258$).

##### 4. Critical Comparison
* **Conventional Survey-based OD Fitting vs. Aggregate TLD-based Multinomial Parameter Identification.**
* **Ontological Separation vs. Experimental Estimands:**
  - **Shapley Attribution (85.8% CPC gain over baseline):** Measures incremental predictive contribution ($\Delta\text{CPC}$) within a 3-component coalition game relative to a uniform-flow baseline.
  - **ANOVA Variance Decomposition ($\eta^2_S = 81.27\%, \eta^2_B = 5.28\%$):** Measures relative contribution to CPC variance under cross-matrix factor perturbation across cities.
  - **Scientific Interpretation:** Neither estimand measures metaphysical or ontological dominance of one component over another. Both provide empirical support for the analytical usefulness of the Structure–Behaviour Separation Principle ($R_S \neq R_B$).
...
| **F4. Transferability of Urban Structure** | Can Urban Structure representations be transferred across geographical contexts? | Structure transferability is an empirical hypothesis. Existing literature demonstrates cross-city generalization of several neural mobility models and representations, but does not establish that a scientifically defined Urban Structure Representation ($R_S$) is universally transferable. Paper 2 therefore tests the transferability of $R_S$ under explicit source–target conditions. | Enaya, A., et al. (2026). TransGM: Transferable gravity models for cross-city policy transfer. *Computers, Environment and Urban Systems*, 128, 102455; Yang, J., et al. (2026). Transferable Human Mobility Network Reconstruction with neuroGravity. *Preprint/SJTU*; Guo et al. (2025); Simini et al. (2021). | Existing studies suggest that neural mobility models can generalize across regions when source and target contexts share relevant spatial characteristics, but the transferability of an independent structural representation ($R_S$) remains an empirical question evaluated under explicit source–target conditions ($CPC = f(\text{structural similarity})$). | This thesis treats cross-city transferability of Urban Structure representations as an empirical hypothesis, while city-specific Travel Behaviour is locally inferred. |
...
* **Spatial Deep Learning Inputs & Tabular Capacity Evidence:** Simini et al. (2021) *DeepGravity* (Nature Communications); Robinson et al. (2021) *Imagery2Flow*. Empirical evaluation of non-spatial tabular baselines (RF, XGBoost, MLP) demonstrates a performance ceiling around $R^2 \approx 0.48$, motivating the investigation of spatial representation learning.
...
* **Non-spatial Tabular Features vs. Spatial Representation Learning:** Evaluated non-spatial tabular baselines show limited performance ($R^2 \approx 0.48$), motivating spatial representation learning. A graph-based architecture is adopted as a candidate mechanism for learning a transferable $R_S$, rather than as a theoretical necessity or uniquely valid architecture.
...
* **Claim G.2:** Existing transfer methods (e.g., DeepGravity, GODDAG, TransGM, Imagery2Flow) transfer end-to-end task-specific neural representations ($Z_{\text{task}}$), jointly encoding structural ($R_S$) and behavioural ($R_B$) information without explicit scientific separation. Crucially, $Z_{\text{task}} \neq R_S$ by default.
...
| **H3. Research Gap I – Behaviour Identification** | Can latent collective travel behavior ($\boldsymbol{\theta}$) be identified from aggregate travel-distance distributions? | Existing studies demonstrate calibration from complete OD data or single summary statistics (e.g., median travel time), but it remains insufficiently established whether full aggregate travel-distance distributions can serve as probabilistic observation layers for city-specific parameter identification. | Merlin, L. A. (2020). A new method using medians to calibrate single-parameter spatial interaction models. *JTLU*; Tanner, J. C. (1961); Hyman, I. (1969); Liang, X., et al. (2013). Unraveling the origin of exponential law. *Scientific Reports*; Gallotti, R., et al. (2024). Distorted insights. *Nature Communications*. | Merlin (2020) demonstrates calibration from a single median travel time; Tanner and Hyman calibrate by matching mean travel length. This study extends aggregate calibration by formulating a multinomial likelihood over the full aggregate distribution. | Identifies Gap I: Prior work calibrates from single summary statistics or relies on full ODs. This thesis formalizes behavioral identification directly from full aggregate TLD distributions under specified structural conditions. |
...
| **H5. Unified Scientific Perspective** | How are the two gaps theoretically connected? | Behavioural Identification (Gap I) and Urban Structure Transferability (Gap II) are complementary scientific problems unified by the Structure–Behaviour Separation Principle. Paper 1 identifies the Travel Behaviour Representation under a specified structural condition, whereas Paper 2 investigates whether the Urban Structure Representation can be learned and transferred independently across cities. Their joint value is evaluated at the integration stage. | Synthesized from Wilson (1971), Haynes & Fotheringham (1984), Simini et al. (2021), and Enaya et al. (2026). | Prior literature provides separate components of spatial friction and opportunities but treats them as joint statistical targets. | Proposes a unified conceptual framework in which Paper 1 and Paper 2 independently evaluate complementary components of the same decomposition before assessing their joint integration. |
| **H6. Transition to the Research Proposal** | What formal scientific framework must be developed next? | These gaps motivate the development of the proposed Structure–Behaviour Separation framework to provide empirical statistical evidence for behavioral identifiability and evaluate downstream planning applications. | Synthesized from the entire Research Knowledge Base (RKB). | Literature identifies limitations of supervised prediction in data-constrained settings, motivating the development of a mathematically bounded, generative reconstruction framework. | Direct transition from the systematic literature review to the formal mathematical and experimental proposal. |
...
* **Claim H.4 (Scientific Significance):** Resolving these gaps enables zero-target-OD mobility reconstruction in data-scarce cities using open urban features and aggregate target-city travel-distance observations, without requiring disaggregated target-city OD matrices or survey microdata.
